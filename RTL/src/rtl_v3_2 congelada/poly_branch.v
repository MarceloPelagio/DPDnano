`timescale 1ns/1ps
`include "config.vh"

module poly_branch
(
    input clk,
    input rst,
    input in_valid,

    input  signed [`TERM_WIDTH-1:0] term_re,
    input  signed [`TERM_WIDTH-1:0] term_im,

    input  signed [`COEF_WIDTH-1:0] coef_re,
    input  signed [`COEF_WIDTH-1:0] coef_im,

output reg out_valid,

    output reg signed [`BRANCH_WIDTH-1:0] branch_re,
    output reg signed [`BRANCH_WIDTH-1:0] branch_im
);

reg stage1_valid;
reg signed [64:0] m0_r, m1_r, m2_r, m3_r;

wire signed [65:0] acc_re = {m0_r[64],m0_r} - {m1_r[64],m1_r};
wire signed [65:0] acc_im = {m2_r[64],m2_r} + {m3_r[64],m3_r};

// Q?.60 -> Q3.30
wire signed [65:0] acc_re_r = acc_re + 66'sd536870912; // 1<<29
wire signed [65:0] acc_im_r = acc_im + 66'sd536870912;

wire signed [`BRANCH_WIDTH-1:0] norm_re = acc_re_r[62:30];
wire signed [`BRANCH_WIDTH-1:0] norm_im = acc_im_r[62:30];

always @(posedge clk or posedge rst) begin
    if(rst) begin
        stage1_valid <= 1'b0;
        m0_r <= 0;
        m1_r <= 0;
        m2_r <= 0;
        m3_r <= 0;
        out_valid <= 1'b0;
        branch_re <= 0;
        branch_im <= 0;
    end else begin
        stage1_valid <= in_valid;
        if(in_valid) begin
            m0_r <= term_re * coef_re;
            m1_r <= term_im * coef_im;
            m2_r <= term_re * coef_im;
            m3_r <= term_im * coef_re;
        end

        out_valid <= stage1_valid;
        if(stage1_valid) begin
            branch_re <= norm_re;
            branch_im <= norm_im;
        end
    end
end

endmodule
