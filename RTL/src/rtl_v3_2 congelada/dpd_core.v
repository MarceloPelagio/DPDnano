...
reg signed [`BRANCH_WIDTH-1:0] lin_re_d2;
reg signed [`BRANCH_WIDTH-1:0] lin_im_d2;
reg signed [`BRANCH_WIDTH-1:0] lin_re_d3;
reg signed [`BRANCH_WIDTH-1:0] lin_im_d3;
reg                            lin_valid_d;
reg                            lin_valid_d2;
reg                            lin_valid_d3;
...
always @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        lin_valid_d2 <= lin_valid_d;
        lin_valid_d3 <= lin_valid_d2;
    end
end
...
always @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        acc_valid <= poly_valid & lin_valid_d3;

        if (poly_valid & lin_valid_d3) begin

            acc_re <=
                {lin_re_d3[`BRANCH_WIDTH-1], lin_re_d3} +
                {poly_re[`BRANCH_WIDTH-1], poly_re};

            acc_im <=
                {lin_im_d3[`BRANCH_WIDTH-1], lin_im_d3} +
