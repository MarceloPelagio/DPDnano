...
def main():
    parser = argparse.ArgumentParser(description="HW023_dpd - stress dinamico com ate 1.000.000 de amostras")
    ...
    with serial.Serial(args.port, 115200, timeout=2.0, write_timeout=1.0) as uart:
        time.sleep(0.1)
        print("DPDnano-Lite HW023_dpd - Stress Dinamico")
        print(f"Porta                 : {args.port}")
        print(f"Amostras              : {args.samples}")
        print("Clock interno DPD     : 60 MHz via PLL")
        print("Fluxo                 : continuo")
        print("Coeficientes          : 5 perfis dinamicos")
        print("Atualizacao           : a cada 256 amostras")
        print("Entrada               : I/Q pseudoaleatoria")
        ...
        command, address, response = transact(uart, CMD_RUN, 0, args.samples)
        if command != CMD_RUN or address != 0 or response != args.samples:
            raise RuntimeError("Falha ao iniciar o teste")

        wall_start = time.perf_counter()
        wait_until_finished(uart, timeout_s=args.timeout)
    ...
    hardware_time_s = (metrics["total_cycles"] / CLOCK_HZ) if metrics["total_cycles"] else 0.0
    hardware_throughput = (received / hardware_time_s) if hardware_time_s else 0.0
    wall_throughput = (received / wall_elapsed) if wall_elapsed else 0.0
        ...
        checkpoint_rows.append(
            {
                ...
                "average_samples_per_cycle": (sample_mark / cycle) if cycle else 0.0,
                "throughput_samples_per_second": (sample_mark * CLOCK_HZ / cycle) if cycle else 0.0,
            }
        )

    checkpoint_csv = output_dir / "hw023_dpd_checkpoints.csv"
    with checkpoint_csv.open("w", newline="", encoding="utf-8") as csv_file:
    ...
    summary_csv = output_dir / "hw023_dpd_stress_summary.csv"
    ...
    print(f"Latencia minima         : {metrics['latency_min']} ciclos")
    print(f"Latencia maxima         : {metrics['latency_max']} ciclos")
    print(f"Latencia media          : {latency_average:.6f} ciclos")
    ...
    print(f"Atualizacoes coef.      : {metrics['coefficient_updates']}")
    print(f"Amostras alta amplitude : {metrics['high_amplitude_samples']}")
    print(f"Overflow                : {metrics['overflow_events']}")
    print(f"Saturacoes positivas    : {metrics['saturation_positive']}")
    print(f"Saturacoes negativas    : {metrics['saturation_negative']}")
    print(f"Assinatura de entrada   : 0x{metrics['input_signature']:08X}")
    print(f"Assinatura de saida     : 0x{metrics['output_signature']:08X}")
    ...
    for row in checkpoint_rows:
        print(f"{row['samples']:7d} amostras  ciclo={row['cycle']:8d}  throughput={row['throughput_samples_per_second']/1e6:.6f} MS/s")

    print("==============================================")
    if passed:
        print("RESULTADO: PASS - stress dinamico de longa duracao validado")
    else:
