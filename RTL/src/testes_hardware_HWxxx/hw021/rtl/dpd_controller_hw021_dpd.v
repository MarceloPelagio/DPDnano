...
/*
 * HW021_dpd - Sensitivity to small coefficient variations
 *
 * Runtime families:
 *
 * Family 1 - coef1 sensitivity:
 *   coef1 = 0.68, 0.69, 0.70, 0.71, 0.72
 *   coef3 = 0.20
 *
 * Family 2 - coef3 sensitivity:
 *   coef1 = 0.70
 *   coef3 = 0.18, 0.19, 0.20, 0.21, 0.22
 *
 * Imaginary coefficients remain zero.
 */

module dpd_controller_hw021_dpd (
    input wire clk,
    input wire rst,
    input wire start_i,
...
always @(*) begin
    case (active_coef1_index)
        3'd0: coef1_re_selected = 16'sh570A; // 0.68
        3'd1: coef1_re_selected = 16'sh5852; // 0.69
        3'd2: coef1_re_selected = 16'sh599A; // 0.70
        3'd3: coef1_re_selected = 16'sh5AE1; // 0.71
        3'd4: coef1_re_selected = 16'sh5C29; // 0.72
        default: coef1_re_selected = 16'sh599A;
    endcase

    case (active_coef3_index)
        3'd0: coef3_re_selected = 16'sh170A; // 0.18
        3'd1: coef3_re_selected = 16'sh1852; // 0.19
        3'd2: coef3_re_selected = 16'sh199A; // 0.20
        3'd3: coef3_re_selected = 16'sh1AE1; // 0.21
        3'd4: coef3_re_selected = 16'sh1C29; // 0.22
        default: coef3_re_selected = 16'sh199A;
    endcase
end
