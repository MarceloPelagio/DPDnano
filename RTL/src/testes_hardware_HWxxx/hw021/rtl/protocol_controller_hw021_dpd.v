...
module protocol_controller_hw021_dpd (
    input wire clk,
    input wire rst_n,
    ...
    output reg [2:0] coef1_index_o,
    output reg [2:0] coef3_index_o,
...
localparam [7:0] CMD_PING=8'h01, CMD_WRITE_IN=8'h20,
                 CMD_READ_OUT=8'h31, CMD_START_DPD=8'h40,
                 CMD_STATUS=8'h41, CMD_COEFS=8'h43;

localparam [31:0] ERR_UNKNOWN=32'hE1, ERR_CSUM=32'hE2,
                  ERR_ADDR=32'hE3, ERR_COUNT=32'hE4,
                  ERR_BUSY=32'hE5, ERR_COEFS=32'hE6;

localparam [3:0] ST_SOF=0,ST_CMD=1,ST_AH=2,ST_AL=3,ST_D3=4,
                 ST_D2=5,ST_D1=6,ST_D0=7,ST_CS=8,
                 ST_READ=9,ST_SEND=10;
...
wire [7:0] request_checksum =
    SOF_REQ^cmd_reg^ah_reg^al_reg^data_reg[31:24]^data_reg[23:16]^
...
task build_response;
    ...
    begin
        ...
        response[8]<=SOF_RSP^command^response_address[15:8]^
                     response_address[7:0]^response_data[31:24]^
                     response_data[23:16]^response_data[15:8]^
                     response_data[7:0];
    end
endtask
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        state<=ST_SOF; cmd_reg<=0; ah_reg<=0; al_reg<=0; data_reg<=0;
        tx_data_o<=0; tx_start_o<=0; tx_index<=0;
        in_wr_en_o<=0; in_wr_addr_o<=0; in_wr_data_o<=0;
        out_rd_en_o<=0; out_rd_addr_o<=0;
        dpd_start_o<=0; dpd_start_addr_o<=0; dpd_count_o<=0;
        coef1_index_o<=2; coef3_index_o<=2;
        busy_o<=0; done_count<=0; error_latched<=0;
    end else begin
        tx_start_o<=0; in_wr_en_o<=0; out_rd_en_o<=0; dpd_start_o<=0;
        ...
        case(state)
            ...
            ST_CS: if(rx_valid_i) begin
                if(rx_data_i!=request_checksum) begin
                    ...
                    error_latched<=1; tx_index<=0; state<=ST_SEND;
                end else begin
                    case(cmd_reg)
                        CMD_PING: begin
                            build_response(CMD_PING,address,0);
                            tx_index<=0; state<=ST_SEND;
                        end
                        ...
                        CMD_WRITE_IN: begin
                            if(dpd_busy_i) begin
                                ...
                                build_response(CMD_WRITE_IN,address,0);
                            end
                            tx_index<=0; state<=ST_SEND;
                        end
                        ...
                        CMD_START_DPD: begin
                            if(dpd_busy_i) begin
                                ...
                                error_latched<=1;
                            end else if(data_reg[8:0]==0 || data_reg[8:0]>256 ||
                                        address+data_reg[8:0]>256) begin
                        ...
                        CMD_STATUS: begin
                            build_response(
                                CMD_STATUS,address,
                                {22'd0,coef3_index_o,coef1_index_o,
                                 dpd_overflow_i,dpd_error_i,dpd_done_i,dpd_busy_i}
                            );
                            tx_index<=0; state<=ST_SEND;
                        end
                        ...
                        CMD_COEFS: begin
                            if(dpd_busy_i) begin
                                build_response(CMD_COEFS,address,ERR_BUSY);
                                error_latched<=1;
                            end else if(data_reg[2:0]>4 || data_reg[6:4]>4) begin
                                build_response(CMD_COEFS,address,ERR_COEFS);
                                error_latched<=1;
                            end else begin
                                coef1_index_o<=data_reg[2:0];
                                coef3_index_o<=data_reg[6:4];
                                build_response(
                                    CMD_COEFS,address,
                                    {25'd0,data_reg[6:4],1'b0,data_reg[2:0]}
                                );
                            end
                            tx_index<=0; state<=ST_SEND;
                        end
            ...
            ST_READ: if(out_rd_valid_i) begin
                build_response(CMD_READ_OUT,address,out_rd_data_i);
                tx_index<=0; state<=ST_SEND;
            end

            ST_SEND: if(!tx_busy_i && !tx_start_o) begin
