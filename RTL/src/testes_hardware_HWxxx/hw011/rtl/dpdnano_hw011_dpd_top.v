...
input_dual_clock_bsram #(.ADDR_WIDTH(8), .DATA_WIDTH(32)) u_input_memory (.wr_clk(clk_27mhz), .wr_en(in_wr_en), .wr_addr(in_wr_addr), .wr_data(in_wr_data), .rd_clk(clk_dpd), .rd_addr(in_dpd_rd_addr), .rd_data(in_dpd_rd_data));
output_dual_clock_bsram #(.ADDR_WIDTH(8), .DATA_WIDTH(32)) u_output_memory (.wr_clk(clk_dpd), .wr_en(out_dpd_wr_en), .wr_addr(out_dpd_wr_addr), .wr_data(out_dpd_wr_data), .rd_clk(clk_27mhz), .rd_addr(out_host_rd_addr), .rd_data(out_host_rd_data));

always @(posedge clk_27mhz or negedge rst_n) begin
    if (!rst_n) begin in_host_rd_valid_r <= 1'b0; out_host_rd_valid_r <= 1'b0; out_host_rd_en_dly1 <= 1'b0; out_host_rd_en_dly2 <= 1'b0; end
    else begin in_host_rd_valid_r <= 1'b0; out_host_rd_en_dly1 <= out_host_rd_en; out_host_rd_en_dly2 <= out_host_rd_en_dly1; out_host_rd_valid_r <= out_host_rd_en_dly2; end
end
always @(posedge clk_dpd or posedge rst_dpd) begin
    if (rst_dpd) begin in_dpd_rd_valid_r <= 1'b0; in_dpd_rd_en_dly1 <= 1'b0; in_dpd_rd_en_dly2 <= 1'b0; end
    else begin in_dpd_rd_en_dly1 <= in_dpd_rd_en; in_dpd_rd_en_dly2 <= in_dpd_rd_en_dly1; in_dpd_rd_valid_r <= in_dpd_rd_en_dly2; end
end
...
dpd_controller_hw010_dpd u_dpd_controller (
    ...
    .out_mem_wr_en_o(out_dpd_wr_en), .out_mem_wr_addr_o(out_dpd_wr_addr), .out_mem_wr_data_o(out_dpd_wr_data),
