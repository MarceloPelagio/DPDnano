...
wire in_host_rd_en, in_host_rd_valid;
wire [7:0] in_host_rd_addr;
wire [31:0] in_host_rd_data;
...
wire out_host_rd_en, out_host_rd_valid;
...
protocol_controller_hw009_dpd u_protocol (
    ...
    .in_wr_en_o(in_wr_en), .in_wr_addr_o(in_wr_addr),
    .in_wr_data_o(in_wr_data),
    ...
    .dpd_count_o(count_uart), .dpd_busy_i(dpd_busy_uart),
    .dpd_done_i(dpd_done_uart), .dpd_error_i(dpd_error_uart),
    .dpd_overflow_i(dpd_overflow_uart),
    .busy_o(protocol_busy), .done_o(protocol_done),
    .error_o(protocol_error)
);

command_cdc_hw_dpd u_command_cdc (
    .src_clk(clk_27mhz), .src_rst(~rst_n),
    .src_start(start_uart), .src_start_addr(start_addr_uart), .src_count(count_uart),
    .dst_clk(clk_dpd), .dst_rst(rst_dpd),
    .dst_start(start_dpd), .dst_start_addr(start_addr_dpd), .dst_count(count_dpd)
);

status_cdc_hw_dpd u_status_cdc (
    .src_clk(clk_dpd), .src_rst(rst_dpd),
    .src_busy(dpd_busy_fast), .src_done(dpd_done_fast),
    .src_error(dpd_error_fast), .src_overflow(dpd_overflow_fast),
    .dst_clk(clk_27mhz), .dst_rst(~rst_n),
    .dst_busy(dpd_busy_uart), .dst_done(dpd_done_uart),
    .dst_error(dpd_error_uart), .dst_overflow(dpd_overflow_uart)
);

input_dual_clock_bsram #(.ADDR_WIDTH(8), .DATA_WIDTH(32)) u_input_memory (
    .wr_clk(clk_27mhz), .wr_en(in_wr_en), .wr_addr(in_wr_addr), .wr_data(in_wr_data),
    .rd_clk(clk_dpd), .rd_addr(in_dpd_rd_addr), .rd_data(in_dpd_rd_data)
);

output_dual_clock_bsram #(.ADDR_WIDTH(8), .DATA_WIDTH(32)) u_output_memory (
    .wr_clk(clk_dpd), .wr_en(out_dpd_wr_en), .wr_addr(out_dpd_wr_addr), .wr_data(out_dpd_wr_data),
    .rd_clk(clk_27mhz), .rd_addr(out_host_rd_addr), .rd_data(out_host_rd_data)
);

dpd_controller_hw013_dpd u_dpd_controller (
    .clk(clk_dpd), .rst(rst_dpd),
    .start_i(start_dpd), .start_addr_i(start_addr_dpd),
    .count_i(count_dpd),
    ...
    .busy_o(dpd_busy_fast), .done_o(dpd_done_fast), .error_o(dpd_error_fast),
