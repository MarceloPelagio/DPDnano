...
CMD_PING, CMD_RUN, CMD_STATUS, CMD_METRIC, CMD_CHECKPOINT = 0x01, 0x60, 0x61, 0x62, 0x63
CLOCK_HZ = 60_000_000
...
def wait_done(uart, timeout_s):
    ...
    while time.time() < deadline:
        ...
        if not busy:
            return
        time.sleep(0.01)
    raise RuntimeError("Timeout esperando execucao")
...
def read_checkpoints(uart):
    ...
    for index in range(10):
        ...
        for _ in range(4):
            command, response_address, value = transact(uart, CMD_CHECKPOINT, index, 0)
            if command != CMD_CHECKPOINT or response_address != index:
                raise RuntimeError(f"Resposta invalida do checkpoint {index}")
...
def main():
    ...
    with serial.Serial(args.port, 115200, timeout=2.0, write_timeout=1.0) as uart:
        ...
        print(f"Execucoes             : {args.runs}")
        print(f"Amostras por execucao : {args.samples}")
        print(f"Amostras totais       : {args.runs * args.samples}")
        print("Clock interno DPD     : 60 MHz")
        print("Bitstream esperado    : HW023")
        print()
        ...
        for run_index in range(1, args.runs + 1):
            ...
            if response != (CMD_RUN, 0, args.samples):
                raise RuntimeError(f"Falha ao iniciar execucao {run_index}")
            ...
            hardware_time = metrics["total_cycles"] / CLOCK_HZ if metrics["total_cycles"] else 0.0
            throughput = received / hardware_time if hardware_time else 0.0
            ...
            passed = (
                ...
                and (metrics["saturation_positive"] + metrics["saturation_negative"]) > 0
            )
    ...
    checkpoints_csv = output_dir / "hw024_dpd_reproducibility_checkpoints.csv"
    with checkpoints_csv.open("w", newline="", encoding="utf-8") as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=list(checkpoint_rows[0].keys()))
        writer.writeheader()
    ...
    saturations = [int(row["saturation_positive"]) + int(row["saturation_negative"]) for row in runs]
    ...
    summary = {
        ...
        "input_signatures_identical": int(len(set(input_signatures)) == 1),
        "output_signatures_identical": int(len(set(output_signatures)) == 1),
    }
    ...
    print()
    print("==============================================")
    print("HW024_dpd - RESUMO ESTATISTICO")
    print("==============================================")
    print(f"Execucoes                 : {args.runs}")
    print(f"PASS                      : {pass_count}")
    print(f"FAIL                      : {args.runs - pass_count}")
    print(f"Taxa de sucesso           : {summary['success_rate_percent']:.2f}%")
    print(f"Latencia media            : {summary['latency_mean_cycles']:.6f}")
    print(f"Desvio padrao latencia    : {summary['latency_stddev_cycles']:.6f}")
    print(f"Throughput medio          : {summary['throughput_mean_msps']:.6f} MS/s")
    print(f"Throughput minimo         : {summary['throughput_min_msps']:.6f} MS/s")
    print(f"Throughput maximo         : {summary['throughput_max_msps']:.6f} MS/s")
    print(f"Desvio padrao throughput  : {summary['throughput_stddev_msps']:.9f}")
    print(f"Overflow medio            : {summary['overflow_mean']:.3f}")
    print(f"Desvio padrao overflow    : {summary['overflow_stddev']:.3f}")
    print(f"Saturacoes medias         : {summary['saturation_mean']:.3f}")
    print(f"Desvio padrao saturacoes  : {summary['saturation_stddev']:.3f}")
    print("Assinaturas entrada iguais: " + ("SIM" if summary["input_signatures_identical"] else "NAO"))
    print("Assinaturas saida iguais  : " + ("SIM" if summary["output_signatures_identical"] else "NAO"))
    print(f"Runs CSV                  : {runs_csv.resolve()}")
    print(f"Summary CSV               : {summary_csv.resolve()}")
    print("==============================================")

    if all_passed:
        if summary["output_signatures_identical"] == 1:
            print("RESULTADO: PASS - reprodutibilidade bit a bit e robustez estatistica validadas")
        else:
            print("RESULTADO: PASS - reprodutibilidade estatistica validada; assinatura agregada de saida variou entre execucoes")
    else:
        print("RESULTADO: FAIL - divergencia entre execucoes")
        raise SystemExit(1)
