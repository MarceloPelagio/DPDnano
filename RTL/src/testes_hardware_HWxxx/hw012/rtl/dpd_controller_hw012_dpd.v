...
*
* Coefficients:
*   coef1 = 0.30 + j0 = 16'sh2666
*   coef3 = 0.70 + j0 = 16'sh599A
******************************************************************************/
...
localparam BYPASS_DIAGNOSTIC = 1'b0;
...
reg signed [`DATA_WIDTH-1:0] sample_re_hold;
reg signed [`DATA_WIDTH-1:0] sample_im_hold;
...
localparam signed [`COEF_WIDTH-1:0] COEF1_RE = 16'sh2666;
localparam signed [`COEF_WIDTH-1:0] COEF1_IM = 16'sh0000;
localparam signed [`COEF_WIDTH-1:0] COEF3_RE = 16'sh599A;
        ...
        case (state)
            ...
            ST_READ: begin
                if (in_mem_rd_valid_i) begin
                    sample_re_hold <= in_mem_rd_data_i[31:16];
                    sample_im_hold <= in_mem_rd_data_i[15:0];
            ...
            ST_FEED: begin
                if (BYPASS_DIAGNOSTIC) begin
                    out_mem_wr_addr_o <= current_addr;
                    out_mem_wr_data_o <= {sample_re_hold, sample_im_hold};
                    state             <= ST_WRITE;
                end else begin
                    dpd_in_valid <= 1'b1;
                    state        <= ST_WAIT_DPD;
