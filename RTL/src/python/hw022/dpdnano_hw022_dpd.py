...
def wait_done(uart, timeout_s=5.0):
    ...
    # A resposta UART do CMD_RUN demora muito mais que o ensaio
    # interno da FPGA. Ao chegar aqui, o engine pode já ter terminado.
    while time.time() < deadline:
        ...
        if error:
            raise RuntimeError(
                "Engine temporal sinalizou erro"
            )

        # Não depender do pulso done, que dura apenas um clock.
        if not busy:
            return overflow
...
def reconstruct_input_cycles(
    ...
    # For random traffic the FPGA uses a deterministic LFSR.
    lfsr=0xACE1
...
with serial.Serial(
    ...
    print("DPDnano-Lite HW022_dpd vr02 - Caracterização Temporal")
    print()
    ...
    for scenario_name,scenario_id,sample_count in SCENARIOS:
        ...
        if command!=CMD_RUN or address!=0 or response!=run_data:
            raise RuntimeError(
                f"Falha ao iniciar cenário {scenario_name}"
            )
        ...
        input_cycles=reconstruct_input_cycles(
            scenario_name,
            min(sample_count,32),
        )
        ...
        print(f"Latência mínima          : {metrics['latency_min']}")
        print(f"Latência máxima          : {metrics['latency_max']}")
        print(f"Latência média           : {latency_average:.6f}")
...
if all_passed:
    print(
        "RESULTADO: PASS - pipeline temporal "
